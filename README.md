...
**حدود المشروع:** التنفيذ تجربة تعليمية محلية على بيانات اصطناعية صغيرة؛ لا يثبت جاهزية إنتاجية أو أداءً موزعًا أو تحمل أعطال Kafka، ولا يتضمن نموذج ذكاء اصطناعي مدربًا أو قياس دقة تنبؤ. تهيئة Colab خاصة بهذه الجلسة، والقياسات المحلية لا تُعمّم على البيئات السحابية. وصل دفتر التشغيل المجمع وحُفظ الأصل واستُخرجت منه دفاتر الأيام الخمسة. وصلت مخرجات محاولة الجودة الجديدة بنجاح. وصل أيضًا فحص إعداد اليوم الخامس الحالي، وأصبحت خلايا الكود الأربعون في الدفتر المجمع ذات مخرجات محفوظة. ووصل ZIP محاولة الجودة الجديدة وفُحص؛ ويبقى رابط مشاركة الأرشيف الكامل وإثبات إعادة تشغيل المسار كاملًا من نسخة نظيفة غير متاحين، كما توضح [قائمة الاستكمال](SUBMISSION_STATUS.md).
...
لم تُرفق مواصفات CPU والذاكرة الدقيقة أو SHA لتعديل الكود وقت التشغيل الأصلي. أما محاولة الجودة الجديدة، فتستخدم حزمة استعادة تتحقق من SHA الكود `a08f7c92f1929ea2b7b2f73842a00b8bfa30628a`؛ كودها محفوظ في [scripts/colab_recovery](scripts/colab_recovery). لا يُستنتج أي منهما من اسم مساحة العمل أو معرّف تشغيل اللاب.
...
على Windows يستخدم `py -3.11 scripts/verify_submission_evidence.py` بدل `python3`. يتحقق السكربت بالمكتبة القياسية من بصمات 171 ملف دليل أصليًا و31 ملفًا أصليًا إضافيًا لمحاولة الجودة الجديدة، وبصمات ملفات المصدر العشرة، وصادرات CSV الثمانية، وتسوية الرحلات والأجور والأحداث والمدد. [نتيجة الفحص المحفوظة](reports/review/retained_artifact_verification.json) تعرض فروقًا صفرية. نُفذ هذا الفحص على الملفات المحلية المطابقة للمحتوى المعد للنشر؛ **لم يُنفذ المسار الكامل من استنساخ نظيف**، ولا يختبر السكربت Spark أو Kafka أو dbt أو اكتمال الدفاتر.

## الدفتر المجمع والنسخ اليومية

وصل [دفتر سلطان المجمع الأصلي](notebooks/Sultan_Training_Project.ipynb)، في نسخته الأخيرة ذات 87 خلية تغطي الأيام الخمسة. حُفظ دون تغيير، واستُخرجت أقسامه في `day01/STUDENT.ipynb` إلى `day05/STUDENT.ipynb` بدل النسخ المرجعية. تطابقت 14 مقارنة لمخرجاته مع التقارير المرفقة، ومنها مساحة العمل `day01_bronze_jmbv27q9` وقياسات الأداء ونتائج التدفق والتقديم.

| اليوم | خلايا الكود | ذات المخرجات المحفوظة | الملاحظة |
|---|---:|---:|---|
| 1 | 18 | 18 | المصدر وBronze والتكلفة والقياس |
| 2 | 6 | 6 | التهيئة وSilver وdbt |
| 3 | 4 | 4 | المعاملات والصيانة |
| 4 | 6 | 6 | نجاح Kafka السابق محفوظ؛ الاستعادة ومحاولة الجودة الجديدة ناجحتان ولهما مخرجات |
| 5 | 4 | 4 | فحص الإعداد الحالي ناجح؛ نتائج اللابين 07 و08 محفوظة من تشغيل سابق |

تتضمن النسخة الجديدة مخرجات نجاح محاولة جودة بعد استعادة 504 ملفات، مع Python 3.11.13، وتسعة فحوص ناجحة، وسبعة صفوف معزولة و75 صفًا معتمدًا. [تفاصيل المحاولة](reports/review/quality_rerun_notebook.json). وصل أرشيفها وفُحصت تقارير GX وبصمات ملفات Delta وقراءة Parquet المستقلة: 82 مرشحًا و7 معزولات و75 معتمدًا؛ تطابقت النتائج مع الدفتر. [نتيجة مراجعة الأرشيف](reports/review/quality_recovery_archive_review.json). بقية مخرجات اللابات محفوظة من التشغيل السابق؛ لم يُعد المستخدم تشغيلها في هذه المحاولة.

تحتوي النسخة الأخيرة `(3)` أيضًا فحص إعداد اليوم الخامس بعد استعادة 631 ملفًا: Python 3.11.13 وJava 17.0.20 وتحقق من تقريري التدفق والجودة المحفوظين. لم يُعد هذا الفحص تشغيل اللابين 07 و08؛ مخرجاتهما التاريخية بقيت كما هي. [سجل فحص الإعداد](reports/review/day05_setup_notebook.json).

التقسيم تنظيمي ولم يتضمن إعادة تنفيذ أو إنشاء مخرجات. كان العمل في دفتر مجمع، وليس خمس جلسات مستقلة. بيانات اعتماد تشغيل الدورة القديمة كانت موروثة داخل metadata؛ أزيلت تلك البيانات من المقتطفات وبقي الأصل محفوظًا. [تفاصيل المصدر والحدود](notebooks/README.md).
...
تُنفّذ هذه الأوامر في بيئة المضيف المجهز. في تشغيل Colab محل المراجعة أُعدّ وسيط Java محلي، ثم شُغّل تمرين التدفق في عملية Python جديدة لتحميل موصل Spark–Kafka منذ البداية. حُفظت خلايا تهيئة Kafka وتشغيل Spark في عملية جديدة، مع مخرجات نجاح التدفق، ضمن [دفتر اليوم الرابع](day04/STUDENT.ipynb) المستخرج من دفتر المستخدم المجمع. لا تعاود اليوم الثاني على جدول Silver المصحح لليوم الثالث، ولا تعاود نشر رسائل قديمة لمجرد إعادة إنشاء التقارير.
...
- أُتيحت التقارير الصغيرة، ومخرجات فحص المصدر والتكلفة، وتوثيق dbt، وخطط Spark، وسجلات الإزاحات، ونتائج GX، وصادرات CSV في [فهرس الأدلة](reports/README.md). أصبحت دفاتر الأيام الخمسة مقتطفات من دفتر المستخدم الفعلي، مع حفظ المخرجات كما وردت. وصلت مخرجات الجودة الجديدة وفحص إعداد اليوم الخامس الحالي؛ لجميع خلايا الكود مخرجات محفوظة. فُحص ZIP محاولة الجودة الجديدة ونُشرت تقاريره؛ ويبقى رابط الأرشيف الكامل وإعادة تشغيل كاملة موثقة من نسخة نظيفة وإجراءات التسليم النهائية. راجع [مراجعة الدفتر](notebooks/README.md). التفاصيل في [SUBMISSION_STATUS.md](SUBMISSION_STATUS.md).
