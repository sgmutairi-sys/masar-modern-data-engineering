...
تتضمن النسخة الجديدة مخرجات نجاح محاولة جودة بعد استعادة 504 ملفات، مع Python 3.11.13، وتسعة فحوص ناجحة، وسبعة صفوف معزولة و75 صفًا معتمدًا. [تفاصيل المحاولة وحدود التحقق](reports/review/quality_rerun_notebook.json). بقية مخرجات اللابات محفوظة من التشغيل السابق؛ لم يُعد المستخدم تشغيلها في هذه المحاولة.
...
- أُتيحت التقارير الصغيرة، ومخرجات فحص المصدر والتكلفة، وتوثيق dbt، وخطط Spark، وسجلات الإزاحات، ونتائج GX، وصادرات CSV في [فهرس الأدلة](reports/README.md). أصبحت دفاتر الأيام الخمسة مقتطفات من دفتر المستخدم الفعلي، مع حفظ المخرجات كما وردت. وصلت مخرجات الجودة الجديدة؛ تبقى خلية إعداد اليوم الخامس بلا مخرجات، وZIP محاولة الجودة الجديدة غير مرفق، ورابط الأرشيف الكامل وإعادة تشغيل كاملة موثقة من نسخة نظيفة وإجراءات التسليم النهائية. راجع [مراجعة الدفتر](notebooks/README.md). التفاصيل في [SUBMISSION_STATUS.md](SUBMISSION_STATUS.md).
